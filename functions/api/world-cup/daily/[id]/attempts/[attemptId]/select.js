...
export async function onRequestPost(context) {
  const rateLimit = checkRateLimit(context.request, "api:worldcup-daily-select", { limit: 20, windowMs: 60_000 });
  if (!rateLimit.allowed) {
    return errorResponse(429, "Too many World Cup daily selections. Please try again shortly.", {
  ...
  try {
    ...
    const selectedPlayer = WORLD_CUP_PLAYER_BY_ID.get(payload.selectedPlayerId);
    if (!selectedPlayer) {
    ...
    return errorResponse(error.status ?? 400, error instanceof Error ? error.message : "Could not lock that World Cup selection.");
  }
}
