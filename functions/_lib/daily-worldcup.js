...
export function validateDailyParticipantId(value) {
  const normalized = String(value ?? "").trim();
...
export function validateDailyStartPayload(payload) {
...
export function validateDailySelectPayload(payload) {
  const body = payload && typeof payload === "object" && !Array.isArray(payload)
    ? payload
    : null;
...
export function validateDailySimulatePayload(payload) {
  const body = payload && typeof payload === "object" && !Array.isArray(payload)
    ? payload
    : null;
...
export function assertDailyAttemptOwnership(definition, attempt, participantId) {
  if (!attempt) {
...
export function buildDailyAttemptResponse(definition, attempt, selections, completedRankedAttempts = null) {
...
export function buildDailySimulationResult(definition, selections) {
  ...
  return buildSingleLimitedOversMatch(completedXI, oppositionLineup, definition.conditions, seed);
}
