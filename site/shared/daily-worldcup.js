...
const CATALOG_PLAYER_BY_SQUAD_AND_STABLE_ID = new Map(
  WORLD_CUP_CATALOG.map((player) => [`${player.squadId}:${player.stableId}`, player]),
);
const BEST_WORLD_CUP_PLAYERS = [...BEST_WORLD_CUP_PLAYER_BY_STABLE_ID.values()];
...
function pickFixedAssignments(slotIndexes, seedText) {
  ...
  for (const slotIndex of orderedSlotIndexes) {
    ...
    const candidates = BEST_WORLD_CUP_PLAYERS
...
function pickDailyRolls(fixedAssignments, seedText) {
  ...
  const squads = shuffle([...WORLD_CUP_SQUAD_BY_ID.values()], squadRng)
...
function buildChallengeDefinition(date, config) {
  ...
  return {
    ...
    date,
    label: `Daily World Cup Challenge · ${date}`,
...
const DAILY_CHALLENGE_SCHEDULE = [
  buildChallengeDefinition("2026-08-06", {
    fixedAssignments: [
...
function buildGeneratedChallenge(date) {
  ...
  for (let variant = 0; variant < 48; variant += 1) {
    ...
    const definition = buildChallengeDefinition(dateText, {
      fixedAssignments,
      oppositionStableIds,
      oppositionLabel: "Historic World Cup challengers",
...
export function getDailyChallengeById(challengeId) {
  ...
  const match = normalizedId.match(/^daily-worldcup-(\d{4}-\d{2}-\d{2})$/u);
...
export function resolveBestWorldCupPlayer(stableId) {
  return BEST_WORLD_CUP_PLAYER_BY_STABLE_ID.get(stableId) ?? null;
}
...
export function getDailyFixedPlayers(definition) {
  return (definition.fixedAssignments ?? [])
    .map((assignment) => {
      const player = resolveBestWorldCupPlayer(assignment.stableId);
...
export function getDailyOppositionPlayers(definition) {
  return definition.oppositionStableIds
    .map((stableId) => resolveBestWorldCupPlayer(stableId))
    .filter(Boolean);
}
...
export function buildDailyPlayerPool(definition, selections) {
  ...
  const selectedPlayers = normalizeDailySelections(selections)
    .map((selection) => WORLD_CUP_PLAYER_BY_ID.get(selection.playerId) ?? resolveBestWorldCupPlayer(selection.stableId))
    .filter(Boolean);
...
export function buildDailyCompletedXI(definition, selections) {
  ...
  if (allHaveSlots) {
    ...
    for (const selection of normalizedSelections) {
      const player = WORLD_CUP_PLAYER_BY_ID.get(selection.playerId);
...
function validSlotIndexesForPlayer(definition, selections, rollNumber, playerId) {
  ...
  const player = WORLD_CUP_PLAYER_BY_ID.get(playerId);
...
export function buildDailyRollPublicState(definition, rollNumber, selections) {
  const roll = getDailyRoll(definition, rollNumber);
  if (!roll) return null;
  const squad = WORLD_CUP_SQUAD_BY_ID.get(roll.squadId);
...
export function buildDailyRecap(definition, selections) {
  ...
  return definition.rolls.map((roll) => {
    ...
    const selectedPlayer = selected ? WORLD_CUP_PLAYER_BY_ID.get(selected.playerId) ?? null : null;
    return {
      rollNumber: roll.rollNumber,
      squadId: roll.squadId,
...
export function buildDailyCommunityStats(definition, completedRankedAttempts, userSelections) {
  ...
  const rolls = definition.rolls.map((roll) => {
    ...
    return {
      rollNumber: roll.rollNumber,
      squadLabel: WORLD_CUP_SQUAD_BY_ID.get(roll.squadId)?.label ?? roll.squadId,
      selections: players,
  ...
  const userSelectionShares = normalizeDailySelections(userSelections)
    .map((selection) => {
      ...
      return {
        ...
        player: WORLD_CUP_PLAYER_BY_ID.get(selection.playerId) ?? null,
